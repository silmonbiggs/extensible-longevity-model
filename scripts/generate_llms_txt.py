...
def generate_brief(stages):
    ...
    lines.append("")
    lines.append("The elm/uncertainty.py module provides Monte Carlo uncertainty")
    lines.append("bounds. Key predictions with 90% confidence intervals (1,000")
    lines.append("samples, CV_lifespan=12%, CV_trajectory=15%):")
    lines.append("")
    lines.append("  - Rapamycin male: +23% (90% CI: 0-49%)")
    lines.append("  - Rapa+Acarb male: +33% (90% CI: 8-61%)")
    lines.append("  - Rapa+NMN male: +32% (90% CI: 7-60%)")
    lines.append("")
    lines.append("Wide CIs reflect genuine biological variation in mouse lifespan")
    lines.append("(ITP reports ~12% CV across sites). The point estimates are the")
    lines.append("model's best prediction; the CIs show the range of individual")
    lines.append("outcomes. Reproducible: run_monte_carlo(compound, seed=42).")
    lines.append("")
