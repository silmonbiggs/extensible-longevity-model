...
def generate_brief(stages):
    ...
    lines.append("")
    lines.append("  - 8 coupled ODE subsystems (listed above)")
    lines.append("  - 6 ITP compound calibrations (12 sex-specific targets)")
    lines.append("  - 4 sex-difference mechanisms (AMPK saturation, PK, testosterone, estrogen)")
    ...
    lines.append("  3. It must be honest about what the data constrain -- Jacobian SVD")
    lines.append("     shows 10 effective degrees of freedom from 12 targets, leaving")
    lines.append("     a well-conditioned (kappa=14.6) estimation problem.")
    lines.append("  4. It must be extensible -- new data (killifish combinations,")
    lines.append("     additional ITP compounds, tissue-specific endpoints) can")
    lines.append("     recalibrate and extend the model without starting over.")
    lines.append("")
    lines.append("The ~150 parameters are not all free. Literature values, structural")
    lines.append("constraints, and normalization constants fix the majority. The")
    lines.append("remaining estimation problem has been shown to be well-conditioned")
    lines.append("in the observable subspace (Slide A1). Could alternative parameter")
    lines.append("sets in distant basins fit the same 12 targets and make different")
    lines.append("combination predictions? In principle, yes. In practice, the Rapa +")
    lines.append("Acarbose out-of-sample prediction (33% vs 34% observed) provides")
    lines.append("evidence that the current basin is not arbitrary.")
    lines.append("")
    lines.append("The alternative to starting somewhere is not starting. The model is")
    lines.append("designed to anneal toward agreement with biology through an explicit")
    lines.append("experiment -> calibrate -> extend cycle (Slide A25). The Taguchi")
    lines.append("killifish screen is the next turn of that cycle. It will either")
    lines.append("validate the combination predictions or provide the data needed to")
    lines.append("move the simplex to a better basin.")
    lines.append("")
    lines.append("## The Cost of Not Starting")
    lines.append("")
    lines.append("The default reviewer instinct is \"treat with appropriate caution.\"")
    lines.append("But caution relative to what?")
    lines.append("")
    lines.append("If serial reliability holds (Slide 2), single-compound testing")
    lines.append("cannot reveal the information needed to guide multi-compound")
    lines.append("interventions. No amount of single-compound mouse studies -- at")
    lines.append("$1.5M and 3+ years each -- will ever answer whether rapamycin +")
    lines.append("NMN is additive, sub-additive, or superadditive. That question")
    lines.append("requires running the combination.")
    lines.append("")
    lines.append("Meanwhile, tens of thousands of people are already combining")
    lines.append("longevity interventions, guided by intuition, anecdote, and")
    lines.append("qualitative pathway diagrams. That number is growing. They are")
    lines.append("conducting slow, uncontrolled experiments on themselves, without")
    lines.append("a quantitative framework -- because none exists.")
    lines.append("")
    lines.append("The proposed Taguchi killifish screen costs $200K and takes 8")
    lines.append("months. If it confirms the model's combination predictions, those")
    lines.append("results feed a targeted mouse validation. If it contradicts them,")
    lines.append("the model recalibrates against real combination data for the first")
    lines.append("time. Either outcome advances the field more than any number of")
    lines.append("additional single-compound studies can.")
    lines.append("")
    lines.append("The risk of acting on an imperfect model: a $200K experiment")
    lines.append("produces data that may require reinterpretation. The risk of not")
    lines.append("acting: combination guidance never improves, because the")
    lines.append("experiments needed to generate it are never run. The cautious")
    lines.append("position is not to wait. The cautious position is to start")
    lines.append("generating combination data in the fastest, cheapest system")
    lines.append("available, and to build the quantitative framework that millions")
    lines.append("of people will eventually need -- whether or not they know it yet.")
    lines.append("")
    lines.append("This is not a fringe position. Many aging researchers already take")
    lines.append("multi-intervention stacks themselves -- readily observable in any")
    lines.append("long-form interview. Their personal choices reflect what serial")
    lines.append("reliability formalizes: one pathway addressed, seven ignored, is")
    lines.append("most of the problem unsolved. It is time for the field's")
    lines.append("experimental priorities to catch up with its practitioners' private")
    lines.append("convictions. Every year without combination data is another year")
    lines.append("that millions of people -- including the next generation --")
    lines.append("navigate longevity interventions by guesswork.")
    lines.append("")
    lines.append("The path from killifish to mice to humans is real and necessary,")
    lines.append("and we fully acknowledge the validation steps required at each")
    lines.append("stage. But that path cannot begin until someone takes step one.")
    lines.append("")
    lines.append("## Key Results")
    lines.append("")
    lines.append("- Calibration: 12 ITP targets, +/-0.08% mean error, 10 effective DOF")
    lines.append("- Out-of-sample validation: Rapa+Acarb predicted +33%, observed +34% (not fit)")
    ...
    lines.append("  - Rapa+Acarb male: median +33%, individual range 8-61%")
...
def generate_full(stages):
    ...
    lines.append("evaluated -- consists of:")
    lines.append("")
    lines.append("  - 8 coupled ODE subsystems (heteroplasmy, NAD+, DNA damage,")
    lines.append("    AMPK/mTORC1, autophagy, senescence, SASP, methylation)")
    lines.append("  - 6 ITP compound calibrations (12 sex-specific targets)")
    lines.append("  - 4 sex-difference mechanisms (AMPK saturation, PK, testosterone, estrogen)")
    lines.append("  - Dose-response Hill curves (3 fitted, 3 estimated)")
    lines.append("  - BioAge composite -> death threshold")
    ...
    lines.append("")
    lines.append("Parameter census (Slide A16): ~150 total parameters break down as")
    lines.append("26 literature-sourced, 78 estimated within physiological constraints,")
    lines.append("39 structural/normalization constants, and ~25 inactive (cancer + human")
    lines.append("scaling). The effective estimation problem is 10 DOF from 12 targets")
    lines.append("(Slide A1), well-conditioned at kappa=14.6.")
    lines.append("")
    lines.append("## Why a Quantitative Model")
    lines.append("")
    lines.append("The biology of aging has accumulated decades of qualitative pathway")
    ...
    lines.append("an explicit experiment -> calibrate -> extend cycle (Slide A25).")
    lines.append("The Taguchi killifish screen is the next turn of that cycle.")
    lines.append("")
    lines.append("## The Cost of Not Starting")
    lines.append("")
    lines.append("If serial reliability holds (Slide 2), single-compound testing")
    ...
    lines.append("$1.5M and 3+ years each -- will ever answer whether rapamycin +")
    lines.append("NMN is additive, sub-additive, or superadditive. That question")
    lines.append("requires running the combination. Meanwhile, tens of thousands of")
    lines.append("people are already combining longevity interventions without a")
    lines.append("quantitative framework -- because none exists. The proposed Taguchi")
    lines.append("killifish screen costs $200K and 8 months. Either it validates the")
    lines.append("predictions or it provides the data to recalibrate. Either outcome")
    lines.append("advances the field more than additional single-compound studies can.")
    lines.append("Many aging researchers already take multi-intervention stacks")
    lines.append("themselves. It is time for the field's experimental priorities to")
    lines.append("catch up with its practitioners' private convictions. The path from")
    lines.append("killifish to mice to humans is real and necessary, and we fully")
    lines.append("acknowledge the validation steps at each stage. But that path")
    lines.append("cannot begin until someone takes step one.")
    lines.append("")
