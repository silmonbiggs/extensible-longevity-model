...
def generate_brief(stages):
    ...
    lines.append("constraints, and normalization constants fix the majority. One-at-a-time")
    lines.append("perturbation of 141 frozen parameters (+/-10%) shifts no prediction by")
    lines.append("more than 0.65 pp (Slide A9). Could alternative parameter sets in")
    lines.append("distant basins fit the same 12 targets and make different combination")
    lines.append("predictions? In principle, yes. In practice, the Rapa + Acarbose")
    lines.append("out-of-sample prediction (33% vs 34% observed) provides evidence")
...
def generate_full(stages):
    ...
    lines.append("scaling). Six free parameters (one scale factor per compound) are")
    lines.append("calibrated to male ITP data by root-finding.")
    lines.append("")
    lines.append("## Why a Quantitative Model")
    lines.append("")
    lines.append("The biology of aging has accumulated decades of qualitative pathway")
