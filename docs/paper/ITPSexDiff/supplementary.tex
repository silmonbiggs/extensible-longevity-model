...
\title{Supplementary Material:\\Four Biological Mechanisms Predict ITP Sex Differences from Male Data Alone}
...
The ELM contains 168 parameters: 151 pathway rate constants and coupling coefficients (Tables~S3--S10), 6 compound scale factors, 6 compound pathway ratios (Table~S2), and 5 sex mechanism parameters (Table~S11). These fall into five categories based on provenance.
...
Canagliflozin & AMPK & 1.00 & SGLT2 inhibition causes urinary glucose loss, activating AMPK via energy deficit. Single-pathway assignment. Note: the model underpredicts female efficacy, suggesting a second, sex-independent pathway (possibly FGF21/ketogenesis; see main text \S3.1). \\
\hline
...
\subsection{BioAge and Normalization}

Normalization constants are derived from the control simulation at $t = 1.0$ (see \S\ref{sec:normalization}). BioAge weights are swept separately in the weight sensitivity analysis (main text); normalization constants are structural.

\begin{table}[H]
\centering
\caption{BioAge weights and normalization constants.}
\begin{tabular}{|llllr|}
\hline
...
% ============================================================
\section{BioAge Weight Validation Against Published Clocks}
\label{sec:clock_mapping}
% ============================================================

To assess whether equal BioAge weights ($w = 0.25$ per subsystem) are consistent with published biological age clocks, we classified the component biomarkers of three clocks into ELM-equivalent categories and summed their published weights (Table~\ref{tab:clock_mapping}).
...
Normalization constants ($\hat{H}_0, \hat{D}_0, \hat{M}_0, \hat{S}_0$) are derived from the control (no-intervention) simulation evaluated at $t = 1.0$ (median lifespan). This ensures BioAge$(1.0) = 1.0$ by construction. The ODE dynamics are independent of these normalization constants; they serve only to set the death-threshold scale. Changing BioAge weights therefore changes the relative contribution of each subsystem at death without introducing circularity, because the normalization constants are recomputed for each weight vector via \texttt{derive\_normalization\_constants()}.
