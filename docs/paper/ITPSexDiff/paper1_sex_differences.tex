...
\title{Why ITP Longevity Drugs Work Differently\\in Male and Female Mice}
...
\begin{abstract}
Most longevity compounds tested by the NIA Interventions Testing Program work better in male mice than female mice, except rapamycin, which works better in females. We show that a mechanistic model of four coupled aging subsystems, calibrated to six male ITP results (6 free parameters, 162 frozen priors from published biology), predicts all six female outcomes from four sex-specific mechanisms in prior literature---none fitted to female data. Mean absolute error is 1.3 percentage points, a sevenfold improvement over the naive assumption that females respond like males (9.0~pp). The model separately predicts the rapamycin + acarbose combination within 0.2~pp, correctly identifying sub-additivity from shared pathway saturation. Where the model errs largest, it errs instructively: canagliflozin is underpredicted by 4~pp because the model routes the entire effect through AMPK, where females already have higher baseline activity. The canagliflozin error pattern suggests a sex-independent second pathway, consistent with FGF21/ketogenesis, and generates a falsifiable prediction: in FGF21-knockout mice, canagliflozin should lose most of its lifespan benefit.
\end{abstract}
...
The biology of aging has attracted sustained theoretical attention for more than a century, producing a vast literature notable more for breadth than predictive power. The distinction between a theory of aging causation and a quantitative model of aging interventions is the difference between explaining \emph{why} an organism dies and predicting \emph{when}. Reliability theory \citep{Gavrilov2001} makes one piece of the problem precise: organisms are series systems that fail when any one critical subsystem reaches its threshold. The consequence is that fixing one failure mode while leaving others intact produces gains proportional to that mode's contribution and no more. The remaining modes do not vanish; they wait. This framework also predicts that drugs targeting the same subsystem will show diminishing returns in combination, while drugs targeting different subsystems should compound---a prediction that requires a multi-subsystem model to test.
...
The model has 6 free parameters (compound scale factors, calibrated to male ITP data) and 162 frozen priors imported from published biology. Four sex-specific mechanisms, each drawn from independent experimental literature, are applied to the calibrated model to generate female predictions. The female predictions use no additional fitted parameters. Their agreement, or disagreement, with observed female outcomes is the central test of whether the model has captured the right biology.
...
\caption{ELM system architecture with ITP compounds (top) and their entry points (downward dashed diagonal lines) to the aging system. Enzymes and processes (diamonds) route flows that add (black arrows) or subtract (red) from state variables (bold rectangles). Signaling (dashed arrows) affects flow rates. The orange dashed line (*) marks a hypothesized canagliflozin pathway analyzed in Section~3.1.}
\label{fig:system}
\end{figure}
...
\midrule
AMPK & energy stress, compounds & Michaelis--Menten & TSC2 activation, NAMPT boost \\ \hline
TSC2 & AMPK & Hill ($n = 1.5$) & Rheb-GAP, mTORC1 suppression \\ \hline
mTORC1 & TSC2, rapamycin & multiplicative & autophagy inhibition \\ \hline
SIRT1 & NAD$^+$ & Hill ($n = 2.0$) & FOXO3 activation, deacetylation \\ \hline
FOXO3 & SIRT1 & linear & mitophagy, stress response \\ \hline
Autophagy & AMPK, FOXO3 & Michaelis--Menten & damage clearance \\ \hline
SASP & SenCells & proportional & CD38, ROS, paracrine senescence \\ \hline
ROS & heteroplasmy, SASP & additive & DNA damage source \\ \hline
EZH2 & AMPK, SIRT1 & inhibitory & DNMT recruitment, CpG drift \\ \hline
...
\begin{equation}
\frac{dM}{dt} = \underbrace{k_{\text{drift}}(1 + \delta_D D + \delta_{\text{SASP}} \cdot \text{SASP})}_{\text{epigenetic drift, amplified by damage \& SASP}} - \underbrace{k_{\text{EZH2}} \cdot f_{\text{AMPK}}(\text{AMPK}) \cdot M}_{\text{AMPK-mediated maintenance}}
\end{equation}
...
\begin{equation}
\frac{dS}{dt} = \underbrace{k_{\text{entry}} \cdot \frac{D^n}{K_D^n + D^n}}_{\text{damage-driven entry}} + \underbrace{k_{\text{repl}}}_{\text{replicative exhaustion}} - \underbrace{k_{\text{clear}} \cdot S}_{\text{immune clearance}}
\end{equation}

\subsubsection{Biological age and survival}

The four damage channels feed into a single composite biological age $B(t)$. NAD$^+$ is excluded because it acts as a coupling intermediate rather than an independent aging endpoint; its effects are captured through its influence on DNA repair, methylation, and mitophagy:
\begin{equation}
B(t) = \left(\frac{w_H}{\hat{H}_0}\right)\!H \;+\; \left(\frac{w_D}{\hat{D}_0}\right)\!D \;+\; \left(\frac{w_M}{\hat{M}_0}\right)\!M \;+\; \left(\frac{w_S}{\hat{S}_0}\right)\!S
\end{equation}
with equal weights $w_H = w_D = w_M = w_S = 0.25$, adopted as an agnostic default rather than inferred from data, where $\hat{H}_0, \hat{D}_0, \hat{M}_0, \hat{S}_0$ are normalization constants derived from control simulation values at $t = 1.0$ (median lifespan), ensuring $B(1.0) = 1.0$ by construction. Biological age determines the death rate through a Gompertz function: $h(t) = h_0 \exp(\lambda \cdot B(t))$, calibrated to the UM-HET3 control survival curve. We define lifespan extension as the percent change in median survival between treated and control simulations.

The equal-prior weight assignment reflects agnosticism about which subsystem dominates mouse mortality and avoids consuming degrees of freedom on weight tuning. We checked this assumption against three published biological age clocks (GrimAge \citep{Lu2019}, PhenoAge \citep{Levine2018}, DunedinPACE \citep{Belsky2022}) by classifying each clock's biomarkers into ELM subsystems and summing weights per category (Supplementary Table~S13). The clock decomposition does not identify a unique weight vector, but it disfavors extreme ones: setting $w_M = 0.50$, for example, is outside the range implied by both GrimAge (20--30\%) and PhenoAge (5--15\%). All four equal weights (0.25) fall within every clock's implied range. Sensitivity analysis (Section~3.3) confirms that predictions are stable across the full plausible range.
...
Rapamycin & mTORC1 inhib. & mTORC1 & 0.60 & \multirow{2}{*}{0.70} & 0.42 \\
 & & AMPK & 0.40 & & 0.28 \\ \hline
Acarbose & $\alpha$-glucosidase & AMPK & 0.50 & \multirow{2}{*}{0.34} & 0.17 \\
 & & gut microbiome & 0.50 & & 0.17 \\ \hline
17$\alpha$-Estradiol & 5$\alpha$-reductase & AMPK & 0.50 & \multirow{2}{*}{0.35} & 0.18 \\
 & & antioxidant & 0.50 & & 0.18 \\ \hline
Canagliflozin$^\dagger$ & SGLT2 inhib. & AMPK & 1.00 & 0.16 & 0.16 \\ \hline
Aspirin & COX inhib. & antioxidant & 0.50 & \multirow{3}{*}{0.20} & 0.10 \\
 & & AMPK & 0.30 & & 0.06 \\
 & & anti-inflam. & 0.20 & & 0.04 \\ \hline
Glycine & antioxidant & antioxidant & 0.90 & \multirow{2}{*}{0.32} & 0.28 \\
 & & AMPK & 0.10 & & 0.03 \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\small $^\dagger$Dual-action decomposition (antioxidant~0.88 / AMPK~0.12, $\alpha_c = 0.60$) explored in Section~3.1.}
\end{table}
...
\end{equation}
We calibrate to male targets because all six compounds produce non-zero male effects, providing six independent constraints. Only four compounds produce non-zero female effects (17$\alpha$-estradiol and aspirin show zero female benefit), so female calibration would leave two scale factors undetermined. The calibration direction reflects data availability, not biological primacy.

Male predictions are exact by construction (error $< 0.01$~pp). An additional 162 quantities are fixed before calibration: 115 taken from published measurements, 24 set within biologically plausible ranges (including 6 adjusted to produce realistic untreated-mouse biology), and 23 structural constants (normalization, coupling topology). None were adjusted to match ITP intervention outcomes. Full provenance is documented in the Supplementary Material.
...
\textbf{AMPK saturation.} Female AMPK baseline is $b = 1.3\times$ male \citep{Boudaba2018,McInnes2011}. We represent this attenuation with the saturating form $f_{\text{atten}} = 1/(1 + k(b - 1))$; with $b = 1.3$ and $k = 8$, this gives 0.29, meaning females get roughly one-third of the AMPK-mediated benefit that males get. This single functional form governs female response for all three AMPK-activating compounds (acarbose, canagliflozin, glycine). Non-AMPK pathway injections (antioxidant, gut microbiome) pass through the sex modifier at full strength.
...
Using four sex-specific mechanisms drawn from prior literature and fixed before comparison to female ITP outcomes, the model predicts the six female outcomes, with 1.3~pp mean absolute error (four of six within 1~pp; Table~\ref{tab:calibration}). This is the primary result.

\begin{table}[ht]
\centering
\caption{ITP observed vs.\ ELM predicted lifespan extension. Male predictions are calibrated (6 free parameters); female predictions in the main model are out-of-sample tests. $^\dagger$The daggered canagliflozin entry shows the post hoc dual-action variant ($f = 0.88$; Section~3.1, Figure~\ref{fig:cana_sweep}) and is not out-of-sample.}
...
Canagliflozin  & +14.0\% & +14.0\% & +9.0\%  & +5.0\%$^\dagger$  & AMPK saturation \\
...
\textbf{Uncertainty estimates.} ITP studies do not report confidence intervals on median extension. We estimate $\pm 2.5$~pp (1 SE) per compound per sex, based on: ITP sample sizes of $\sim$130--150 treated and $\sim$270 control mice per sex \citep{Miller2007}, bootstrap SE of the median ($1.253 \times \sigma / \sqrt{n}$, with $\sigma \approx 170$~days for UM-HET3 lifespans), and propagation through the treated-minus-control ratio. This is consistent with the ITP's stated power to detect a 10\% change in mean lifespan at 80\% power. Model prediction uncertainty is computed by one-at-a-time perturbation of all 141 frozen parameters ($\pm$10\%), with male scale factors re-calibrated at each perturbation (Supplementary~\S S2.2); per-compound female sensitivities are then scaled by each parameter's plausible range and summed in quadrature (RSS). The resulting model SE ranges from 0~pp (aspirin, 17$\alpha$-estradiol: categorical mechanisms) to 2.3~pp (rapamycin: PK-mediated sensitivity to heteroplasmy parameters).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{sex_diff_baseline.png}
\caption{ELM predictions vs.\ ITP observations for all six compounds, both sexes (pure-AMPK canagliflozin model). For each compound, paired bars show ITP observed (dark) and ELM predicted (light), with blue for males and red for females. Error bars on ITP bars: estimated SE of the median extension ($\pm 2.5$~pp; see text). Error bars on ELM female bars: model parameter uncertainty (OAT-derived RSS; Supplementary~\S S2.2). ELM male bars carry no error bars because each male prediction is solved exactly to its ITP target (one free parameter per compound). Four of six female predictions fall within combined 1$\sigma$ uncertainty; canagliflozin and acarbose exceed it.}
\label{fig:sex_diff}
\end{figure}

\textbf{AMPK saturation (acarbose, canagliflozin, glycine).} A 30\% higher AMPK baseline, on a steeply saturating curve, leaves only 29\% of the male drug response available (attenuation factor 0.29). The per-compound predictions: glycine is predicted well (+3.8\% vs.\ +4.0\%) because 90\% of its injection is antioxidant, which passes through the sex modifier unattenuated. Acarbose is overpredicted (+7.6\% vs.\ +5.0\%, 2.6~pp). Canagliflozin is underpredicted (+5.0\% vs.\ +9.0\%, 4.0~pp).

The opposite-sign errors for acarbose and canagliflozin are informative. If both compounds acted purely through AMPK, their female attenuation should be similar. Instead they err in opposite directions: acarbose is overpredicted, canagliflozin is underpredicted. The model suggests a natural explanation: the AMPK saturation mechanism diminishes only the \texttt{ampk} and \texttt{gut\_microbiome} pathway injections; an antioxidant fraction passes through at full strength for females. If part of canagliflozin's effect operates through a sex-independent pathway, females would retain that component unattenuated, raising the predicted female benefit toward the observed value.

Three independent lines of evidence support a sex-independent second pathway: SGLT2 inhibition shifts substrate utilization toward fatty acid oxidation and ketogenesis \citep{Ferrannini2016}; canagliflozin reprograms systemic metabolism via FGF21-dependent pathways in mice \citep{Osataphan2019}; and SGLT2 inhibitors activate ketogenic starvation sensors independently of glucose lowering \citep{Packer2020}. In the model, this maps to the antioxidant node, which reduces ROS-driven DNA damage---precisely the downstream endpoint of Nrf2 activation.

To quantify this hypothesis, we swept the antioxidant fraction $f$ from 0 to 1, recalibrating the male scale factor at each point via root-finding so that the male prediction remains exactly +14\% regardless of the split (Figure~\ref{fig:cana_sweep}). This introduces one additional parameter, the pathway split $f$, constrained by one datum: female canagliflozin = +9\%. The revised fit is therefore not out-of-sample, but it yields a new falsifiable prediction.

The sweep constrains $f$ but cannot pin it down precisely. The mean female error (Figure~\ref{fig:cana_sweep}B, black) stays below the expected MAE of a perfect model measured against ITP-precision data ($\sigma\sqrt{2/\pi} \approx 2.0$~pp) for the \emph{entire} sweep, so the mean-error minimum at $f^* = 0.88$ is a point estimate within the noise, not a statistically resolved optimum. The maximum single-compound error (red) provides the binding constraint: it drops below the ITP measurement SE (2.5~pp) for $f \in [0.70, 0.96]$ and flatlines at 2.8~pp across that range. In this band, canagliflozin is no longer the worst-predicted compound---acarbose is (+7.8\% predicted vs.\ +5.0\% observed), and no further improvement to the maximum error is possible without addressing acarbose independently.

What the data support is therefore: a dominant sex-independent pathway ($f$ in the range 0.70--0.96), with the AMPK-mediated fraction as a minor contributor. At the point estimate $f^* = 0.88$, the canagliflozin prediction improves from +5.0\% to +8.8\% (0.2~pp error) and mean female MAE drops from 1.3 to 0.65~pp.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{cana_antioxidant_sweep.png}
\caption{Canagliflozin dual-action sweep. \textbf{(A)}~Female lifespan extension as a function of the antioxidant fraction $f$, with male extension recalibrated to +14\% at each point. The dashed line marks the ITP observed value (+9\%). \textbf{(B)}~Mean and maximum female prediction error across all six ITP compounds. Dotted reference lines show the ITP measurement SE (2.5~pp, red) and the expected MAE of a perfect model measured against data of this precision ($\sigma\sqrt{2/\pi} \approx 2.0$~pp, grey). The mean error is below its noise floor for all $f$; the maximum error constrains $f$ to $[0.70, 0.96]$.}
\label{fig:cana_sweep}
\end{figure}

The falsifiable prediction: in FGF21-knockout mice, canagliflozin should retain at most the AMPK-mediated fraction of its lifespan benefit---a residual male extension of 1--4~pp (instead of +14\%), and near-zero female extension (the small AMPK residual is further diminished by AMPK saturation). Plasma FGF21 and $\beta$-hydroxybutyrate in canagliflozin-treated wild-type mice should be elevated to a similar degree in both sexes, confirming the sex-independent nature of the dominant pathway.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{sex_diff_dual_action.png}
\caption{Same layout as Figure~\ref{fig:sex_diff}, with canagliflozin updated to the dual-action model ($f^* = 0.88$). The canagliflozin female gap closes from 4.1~pp to 0.2~pp. Five of six compounds now fall within combined 1$\sigma$ uncertainty; acarbose remains the sole outlier.}
\label{fig:sex_diff_dual}
\end{figure}
...
No sex-specific parameters were adjusted to ITP lifespan data. The AMPK baseline ratio (1.3$\times$) and rapamycin PK ratio (3.48$\times$) come from published measurements; the saturation curvature ($k = 8$) is a round number set near the midpoint of the plausible range [3--20] (Section~4.2). All three were fixed before examining female ITP outcomes. For comparison, a naive null model that assumes female extension equals male extension produces a mean absolute error of 9.0~pp---the four sex mechanisms reduce this sevenfold, to 1.3~pp.
...
The ELM predicts +34\% for the rapamycin + acarbose combination (male, 9-month start); the ITP observed +34\% \citep{Strong2022}. This combination was not used in any parameter fitting: neither the compound scale factors (calibrated to individual male targets) nor the biological age weights (equal priors, fixed before calibration) were adjusted to match this result.
...
At ITP doses, rapamycin alone drives autophagy to $\sim$78\% of maximum; acarbose drives it to $\sim$65\%. Combined, the pathway encounters diminishing returns near saturation. A naive additive model predicts +45\%. The ELM predicts +34\%. The ITP observes +34\% (Figure~\ref{fig:combo}). The naive model is not close. The ELM identifies the direction of the interaction (sub-additive), its approximate magnitude, and the correct mechanistic reason: pathway saturation at a shared node.

\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{combo_rapa_acarb.png}
\caption{Out-of-sample combination prediction: rapamycin + acarbose (male, 9-month start). Individual compound effects and ITP observed in dark blue (data); naive additive sum in gray; ELM prediction in light blue (model). Error bars: $\pm$1.9~pp model SE (ELM), $\pm$2.5~pp estimated SE (ITP). The naive sum (+45\%) overpredicts by 11~pp; the ELM (+34\%) matches the ITP (+34\%) within combined uncertainty.}
\label{fig:combo}
\end{figure}

\subsection{Sensitivity and Robustness}

\textbf{Weight sensitivity.} We swept the methylation weight across $[0.05, 0.50]$, well beyond the biologically plausible clock-decomposition range of $[0.05, 0.30]$ (Supplementary Section~S3). The data cannot strongly constrain this weight: the mean calibration error stays below the ITP measurement noise floor ($\sim$2.5~pp SE) everywhere, and the maximum single-compound error barely exceeds it (Figure~\ref{fig:weight_sweep}A). Equal 25\% weights remain compatible with the data on hand. The out-of-sample combination prediction provides mild additional constraint: the predicted rapa+acarbose extension crosses the observed +34\% near $w_M = 0.25$ and remains within $\pm$2.5~pp (one ITP~SE) for $w_M \in [0.10, 0.40]$ (Figure~\ref{fig:weight_sweep}B).
...
\caption{Biological age weight sensitivity. \textbf{(A)}~Mean and maximum calibration error across all 12 compound$\times$sex predictions as the methylation weight varies from 0.05 to 0.50. The dashed line marks the ITP measurement SE ($\sim$2.5~pp); mean error stays well below this noise floor across the entire range. \textbf{(B)}~Out-of-sample rapa+acarbose combination prediction. The salmon band shows $\pm$2.5~pp (one ITP~SE) around the observed +34\%. The prediction falls within this band for $w_M \in [0.10, 0.40]$. Purple shading marks the biologically plausible clock-decomposition range.}
...
\textbf{AMPK saturation parameter.} The diminishing-returns curvature $k = 8$ is a modeling choice, not a directly measured quantity. It is set as the midpoint of the plausible range for hyperbolic saturation of the LKB1$\to$AMPK$\to$TSC2 cascade. Different values of $k$ shift the AMPK-compound female predictions in concert. The canagliflozin dual-action split ($f^* = 0.88$) was determined at $k = 8$; a different $k$ would shift the optimal $f^*$ but preserve the qualitative conclusion that the dominant canagliflozin pathway is sex-independent.
...
A 6-parameter model, calibrated only to male ITP data, predicts all six female outcomes with 1.3~pp mean absolute error (four of six within 1~pp) using four independently motivated sex mechanisms---and predicts the rapamycin + acarbose combination within 0.2~pp, for the correct mechanistic reason. Where it errs, the pattern of error suggests a second, sex-independent pathway, which the model quantifies ($f^* = 0.88$, consistent with FGF21/ketogenesis) and makes falsifiable (FGF21-knockout males: 1--4~pp residual extension vs.\ +14\% in wild type).
