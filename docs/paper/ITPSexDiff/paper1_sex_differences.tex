...
The model has 6 free parameters (compound scale factors, calibrated to male ITP data) and 162 frozen priors imported from published biology. Four sex-specific mechanisms, each drawn from independent experimental literature, are applied to the calibrated model to generate female predictions. The sex predictions use zero additional fitted parameters. Whether they match the observed female outcomes is the central test of whether the model has captured the right biology.
...
\textbf{AMPK saturation.} Female AMPK baseline is $b = 1.3\times$ male \citep{Boudaba2018,McInnes2011}. We model this saturation as $f_{\text{atten}} = 1/(1 + k(b - 1))$ with $k = 8$, giving a factor of 0.29, meaning females get roughly one-third of the AMPK-mediated benefit that males get. This single functional form governs female response for all three AMPK-activating compounds (acarbose, canagliflozin, glycine). Non-AMPK pathway injections (antioxidant, gut microbiome) pass through the sex modifier at full strength.
...
Using four biological mechanisms drawn from prior literature, none fitted to female data, the model predicts all six female ITP outcomes from male calibration alone, with 1.3~pp mean absolute error (four of six within 1~pp; Table~\ref{tab:calibration}). This is the primary result.

\begin{table}[ht]
\centering
\caption{ITP observed vs.\ ELM predicted lifespan extension. Male predictions are calibrated (6 free parameters); female predictions are genuine out-of-sample tests. $^\dagger$With dual-action pathway split ($f = 0.88$; Section~3.1, Figure~\ref{fig:cana_sweep}), canagliflozin female prediction becomes +8.8\%.}
...
The acarbose--canagliflozin error pattern is diagnostic. If both compounds acted purely through AMPK, their female attenuation should be similar. Instead they err in opposite directions: acarbose is overpredicted, canagliflozin is underpredicted. The model's structure identifies why: the AMPK saturation mechanism diminishes only the \texttt{ampk} and \texttt{gut\_microbiome} pathway injections; an antioxidant fraction passes through at full strength for females. If part of canagliflozin's effect operates through a sex-independent pathway, females would retain that component unattenuated, raising the predicted female benefit toward the observed value.
...
To quantify this hypothesis, we swept the antioxidant fraction $f$ from 0 to 1, recalibrating the male scale factor at each point via root-finding so that the male prediction remains exactly +14\% regardless of the split (Figure~\ref{fig:cana_sweep}). The sweep identifies $f^* = 0.88$ (plausible range 0.75--0.94) as the split that minimizes mean female prediction error across all six compounds. This adds one parameter (the pathway split $f$), identified by one datum (female canagliflozin = +9\%). It is not a free prediction---but it generates one.

If the FGF21 hypothesis is correct, the model-to-data match for canagliflozin improves from +5.0\% to +8.8\% (0.2~pp error), and mean female MAE drops from 1.3 to 0.65~pp, with five of six compounds within 1~pp.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{cana_antioxidant_sweep.png}
\caption{Canagliflozin dual-action sweep. \textbf{(A)}~Female lifespan extension as a function of the antioxidant fraction $f$, with male extension recalibrated to +14\% at each point. The dashed line marks the ITP observed value (+9\%). \textbf{(B)}~Mean and maximum female prediction error across all six ITP compounds. The minimum at $f^* = 0.88$ halves the mean error relative to the pure-AMPK model ($f = 0$).}
...
A 6-parameter model, calibrated only to male ITP data, predicts all six female outcomes with 1.3~pp mean absolute error (four of six within 1~pp) using four independently motivated sex mechanisms---and predicts the rapamycin + acarbose combination within 0.2~pp, for the correct mechanistic reason. Where it errs, it errs instructively: the canagliflozin underprediction diagnoses a second, sex-independent pathway, which the model quantifies ($f^* = 0.88$, consistent with FGF21/ketogenesis) and makes falsifiable (FGF21-knockout males: 1--4~pp residual extension vs.\ +14\% in wild type).
